...
#include <fstream>
#include <sstream>
//#include <tinyxml2.h> // Asegúrate de tener tinyxml2 o usa tu librería XML preferida
...
#endif //PBG_ADD_NEWCHAR_MONK_ANI
{
	...
	int Temp = static_cast<int>(*AnimationFrame);
	*AnimationFrame += Speed;
	if (Temp != static_cast<int>(*AnimationFrame))
	{
		*PriorAction = CurrentAction;
		*PriorAnimationFrame = static_cast<float>(Temp);
	}
	if (*AnimationFrame <= 0.f)
	{
		*AnimationFrame += static_cast<float>(Actions[CurrentAction].NumAnimationKeys - 1.f);
	}

	if (Actions[CurrentAction].Loop)
	{
		if (*AnimationFrame >= static_cast<float>(Actions[CurrentAction].NumAnimationKeys))
		{
			*AnimationFrame = static_cast<float>(Actions[CurrentAction].NumAnimationKeys) - 0.01f;
			Loop = false;
		}
	}
	else
	{
		...
		if (fTemp >= static_cast<int>(Key))
		{
			int Frame = static_cast<int>(*AnimationFrame);
			*AnimationFrame = static_cast<float>(Frame % Key) + (*AnimationFrame - static_cast<float>(Frame));
			Loop = false;
		}
	}
	CurrentAnimation = *AnimationFrame;
	CurrentAnimationFrame = static_cast<int>(CurrentAnimation);
...
void BMD::ExportAllBMDToObjAndXml(const std::string& bmdFolder, const std::string& exportFolder)
{
	namespace fs = std::filesystem;
	fs::create_directory(exportFolder);

	for (const auto& entry : fs::recursive_directory_iterator(bmdFolder))
	{
		if (!entry.is_regular_file())
			continue;

		std::string path = entry.path().string();
		if (entry.path().extension() != ".bmd" && entry.path().extension() != ".BMD")
			continue;

		// Cargar el modelo BMD
		BMD model;
		// Obtener la carpeta relativa para mantener la estructura en Export
		std::string relativePath = fs::relative(entry.path().parent_path(), bmdFolder).string();
		std::string exportSubFolder = exportFolder;
		if (!relativePath.empty() && relativePath != ".")
		{
			exportSubFolder += "/" + relativePath;
			fs::create_directories(exportSubFolder);
		}

		if (!model.Open((char*)entry.path().parent_path().string().c_str(), (char*)entry.path().filename().string().c_str()))
			continue;

		// Exportar a .obj en la subcarpeta correspondiente
		std::string objName = exportSubFolder + "/" + entry.path().stem().string() + ".obj";
		std::ofstream objFile(objName);
		if (!objFile.is_open())
			continue;

		int vertexOffset = 1;
		for (int meshIdx = 0; meshIdx < model.NumMeshs; ++meshIdx)
		{
			Mesh_t& mesh = model.Meshs[meshIdx];

			// Vértices
			for (int v = 0; v < mesh.NumVertices; ++v)
			{
				const Vertex_t& vert = mesh.Vertices[v];
				objFile << "v " << vert.Position[0] << " " << vert.Position[1] << " " << vert.Position[2] << "\n";
			}

			// Normales
			for (int n = 0; n < mesh.NumNormals; ++n)
			{
				const Normal_t& norm = mesh.Normals[n];
				objFile << "vn " << norm.Normal[0] << " " << norm.Normal[1] << " " << norm.Normal[2] << "\n";
			}

			// Coordenadas de textura
			for (int t = 0; t < mesh.NumTexCoords; ++t)
			{
				const TexCoord_t& tex = mesh.TexCoords[t];
				objFile << "vt " << tex.TexCoordU << " " << (1.0f - tex.TexCoordV) << "\n";
			}

			// Caras
			for (int f = 0; f < mesh.NumTriangles; ++f)
			{
				const Triangle_t& tri = mesh.Triangles[f];
				if (tri.Polygon < 3) continue;

				objFile << "f";
				for (int k = 0; k < 3; ++k)
				{
					int vi = tri.VertexIndex[k] + vertexOffset;
					int ti = tri.TexCoordIndex[k] + vertexOffset;
					int ni = tri.NormalIndex[k] + vertexOffset;
					objFile << " " << vi << "/" << ti << "/" << ni;
				}
				objFile << "\n";
			}
