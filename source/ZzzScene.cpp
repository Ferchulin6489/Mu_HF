...
// Variable estática para evitar múltiples mensajes
static bool g_bExportPromptShown = false;
...
char* szServerIpAddress = "45.40.99.135";
WORD g_ServerPort = 44402;
...
void NewMoveLogInScene()
{
	...
	{
		if (!g_bExportPromptShown)
		{
			g_bExportPromptShown = true;

			// Mostrar mensaje de confirmación (adapta esto a tu sistema de UI)
			int respuesta = MessageBoxA(NULL, "¿Desea exportar todos los archivos BMD a OBJ/XML?", "Exportar modelos", MB_YESNO | MB_ICONQUESTION);
			if (respuesta == IDYES)
			{
				// Obtener el directorio de ejecución del juego
				...
				// Construir las rutas absolutas a Data y Export
				std::string dataPath = exeDir + "Data";
				std::string exportPath = exeDir + "Export";

				// Crear una instancia de BMD para llamar al método miembro
				BMD::ExportAllBMDToObjAndXml(dataPath, exportPath);

				MessageBoxA(NULL, "Exportación finalizada.", "Exportar modelos", MB_OK | MB_ICONINFORMATION);
			}
		}
	}
	else
	{
		g_bExportPromptShown = false; // Permite volver a mostrar el mensaje si se suelta F2
	}
...
bool MoveMainCamera()
{
	...
	if (SceneFlag == MAIN_SCENE)
	{
		if (MouseWheel)
		{
			Camera3DFov = std::clamp(Camera3DFov - MouseWheel, -25.f, 5.f);

			MouseWheel = 0;
		}
		CameraFOV = std::clamp(CameraFOV + Camera3DFov, 10.f, 40.f);
...
void MainScene(HDC hDC)
{
	...
	DifTimer = TimePrior - LastTimeCurrent;
	// Eliminar el control de Sleep y el ajuste de tiempo
	// DifTimer += Remain; // Si Remain ya no se usa, puede eliminarse también
...
void Extraer_Data()
{
	...
	extern BMD* Models;
	const int MAX_MODELS = 512; // Ajusta según tu proyecto

	for (int m = 0; m < MAX_MODELS; ++m)
	{
		BMD& model = Models[m];
		if (model.Meshs == nullptr || model.NumMeshs <= 0)
			continue;

		// Nombre del archivo .obj en la carpeta Export
		std::string objName = std::string("Export/") + model.Name + ".obj";
		...
		for (int meshIdx = 0; meshIdx < model.NumMeshs; ++meshIdx)
		{
			...
			for (int f = 0; f < mesh.NumTriangles; ++f)
			{
				...
				for (int k = 0; k < 3; ++k)
				{
					int vi = mesh.Vertices ? tri.VertexIndex[k] + vertexOffset : 0;
					int ti = mesh.TexCoords ? tri.TexCoordIndex[k] + vertexOffset : 0;
					int ni = mesh.Normals ? tri.NormalIndex[k] + vertexOffset : 0;
					objFile << " " << vi;
					if (mesh.TexCoords || mesh.Normals)
					{
						objFile << "/";
						if (mesh.TexCoords) objFile << ti;
						if (mesh.Normals) objFile << "/" << ni;
					}
				}
				objFile << "\n";
			}
